...
def main(args):
    ...
    # 모델을 불러옴(사전학습된 모델)
    print("<사전 학습된 BLIP 모델 가져오기>") # blip_pretrain.py의 init()실행
    model = model_factory(model_name=args.model)
    print("<사전 학습된 BLIP 모델 가져옴>")
    ...
    # ... while some others do :)
        ...
        # General 데이터셋은 SkyScript (모델 학습을 위한 일반적인 이미지-텍스트 데이터셋)
        # Region 데이터셋은 VG (세부적인 지역 정보가 포함된 데이터셋)
        ...
        general_sampler, region_sampler = create_sampler( #샘플링 방식을 설정
        ...
        [general_loader, region_loader] = create_loader( # 데이터를 pytorch에서 사용할 수 있는 loader로 변환 -> 데이터셋을을 학습에 사용할 수 있도록 함함
        ...
        score_args = [model]
        score_kwargs = { # pruning할 때 인스턴스로 사용할 수 있도록 함
    ...
    # Pruner객체 생성(pruner + model 정보를 담음)
    print("<Pruner 객체 생성,  prune.py -> multiflow.py로 이동>")
    pruner = get_pruner_by_name(args.pruner, *init_args, **init_kwargs)

    # set the model in the correct mode according to the pruner
    #pruners/base에 requires_training = False라고 정의되어 있음
